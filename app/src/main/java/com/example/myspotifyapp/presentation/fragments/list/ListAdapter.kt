...
class ListAdapter(private var myList: List<Item>, private var context: Context, private val itemListener: (item: Item) -> Any) : RecyclerView.Adapter<ListAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.binding.itemSongName.text = item.track.name
        holder.binding.itemSongArtist.text = item.track.artists[0].name
        val durationSongMinutos = ((item.track.duration_ms / 1000) / 60).toString()
        ...
        val text= "$durationSongMinutos:$durationSongSegundos"
        holder.binding.itemSongDuration.text = text

        val url = item.track.album.images[0].url
